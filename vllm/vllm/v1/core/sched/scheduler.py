...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # Pending KV copy operations for compaction. Maps request_id to
        # list of (src_block_id, src_offset, dst_block_id, dst_offset) tuples.
        # These are sent to the worker to execute the GPU tensor copies.
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            if (self.block_masking_config is not None and
                self.block_masking_config.debug and
                request.block_masking_state is not None and
                num_new_tokens > 1):
                logger.info(
                    "Block masking re-prefill scheduled for %s: "
                    "%d tokens (num_computed=%d, num_tokens=%d)",
        ...
        # Get pending block table truncations for compaction.
        ...
        if self.block_masking_processor is not None:
            ...
            for req_id in num_scheduled_tokens:
                ...
                if (request is not None and 
                    request.block_masking_state is not None and
                    request.block_masking_state.pending_compactions and
                    not request.block_masking_state.deferred_compaction_scheduled and
                    len(request.output_token_ids) == 0):
                    # This request has pending compactions and no output yet
                    # It will complete prefill this step - skip sampling
                    # Mark as scheduled to avoid skipping multiple times
                    request.block_masking_state.deferred_compaction_scheduled = True
                    skip_sampling_req_ids.add(req_id)
                    if (self.block_masking_config is not None and
                        self.block_masking_config.debug):
                        logger.info(
                            "Block masking: skipping sampling for %s "
                            "(pending compactions, first decode)",
                            req_id
                        )
    ...
    def _preempt_request(
        ...
        # Reset block masking / compaction state so that re-prefill
        # rebuilds it from scratch (matches the now-empty KV cache).
    ...
    def _make_cached_request_data(
        ...
        for idx, req in enumerate(itertools.chain(running_reqs, resumed_reqs)):
            ...
            # Track how many tokens have been compacted (physically removed)
            # for this request. Used by worker to compute physical seq_lens.
            compacted_token_counts.append(req.get_compacted_token_count())
                ...
                if state.pending_compactions:
                    ...
                    while compaction:
                        ...
                        if (self.block_masking_config is not None and
                            self.block_masking_config.debug):
                            logger.info(
                                "Block masking deferred compaction for %s: [%d, %d) -> %s",
                                request.request_id, start, end, result
                            )
                        # Restart mode: rewind for summary recomputation
                        if (state.pending_restart_rewind is not None):
                            request.num_computed_tokens = (
                                state.pending_restart_rewind)
                            # Reset async scheduling placeholder count so
                            # num_new_tokens is computed correctly after rewind.
                            request.num_output_placeholders = 0
                            if (self.block_masking_config is not None and
                                self.block_masking_config.debug):
                                logger.info(
    ...
    def _update_request_with_output(
        ...
        # Block masking: Execute pending compactions from prompt BEFORE
        # processing the first generated token. This ensures subsequent
        # tokens can't attend to masked block content.
        if (self.block_masking_processor is not None and 
            request.block_masking_state is not None and
            len(request.output_token_ids) == 0 and
            new_token_ids):
            # First generated token - compact any blocks found in prompt
            state = request.block_masking_state
            if state.pending_compactions:
                compaction = state.get_compaction_range(
                    self.block_masking_config.keep_last_n_blocks,
                ...
                while compaction:
                    ...
                    if (self.block_masking_config is not None and
                        self.block_masking_config.debug):
                        logger.info(
                            "Block masking prompt compaction for %s: [%d, %d) -> %s",
                            request.request_id, start, end, result
                        )
                    # Restart mode: rewind for summary recomputation
                    if (state.pending_restart_rewind is not None):
        ...
        for num_new, output_token_id in enumerate(new_token_ids, 1):
            ...
            if (self.block_masking_processor is not None and 
                request.block_masking_state is not None):
                # Calculate absolute position (prompt + output)
                ...
                if compaction:
                    start, end = compaction
                    # Trigger compaction using existing mask_token_span
                    result = self.mask_token_span(
                        request.request_id,
                        start,
                        end,
                    ...
                    if (self.block_masking_config is not None and
                        self.block_masking_config.debug):
                        logger.info(
                            "Block masking compaction for %s: [%d, %d) -> %s",
                            request.request_id, start, end, result
                        )
                    # Restart mode: rewind num_computed_tokens to recompute
                    # summary KV without block content in the cache.
                    if (request.block_masking_state.pending_restart_rewind
                        ...
                        request.num_computed_tokens = rewind_pos
                        request.num_output_placeholders = 0
                        request.block_masking_state.pending_restart_rewind = (
                            None)
    ...
    def add_request(self, request: Request) -> None:
        ...
        if self.block_masking_processor is not None:
            ...
            # Process prompt tokens to detect existing block structures
            if request.prompt_token_ids:
                self.block_masking_processor.process_prompt_tokens(
                    request.block_masking_state,
    ...
    def mask_token_span(
        ...
        Computes the physical slot positions of active tokens using the
        current compacted_spans (before adding the new span), passes them
        to the KV cache manager for compaction, then merges the new span
        into compacted_spans.
        ...
        Note:
            - Masked tokens will not be attended to by subsequent generation
            - KV cache is compacted to physically remove masked tokens
            - This operation is irreversible (cannot unmask)
            - Sequential calls are safe: physical positions are correctly
              computed from the current compacted_spans before each call.
        """
        ...
        # Compute all inactive logical positions (existing compacted + new).
        all_inactive = self._merge_spans(
            request.compacted_spans + [new_span]
        )

        # Determine active logical positions by iterating through gaps
        # between sorted, non-overlapping inactive spans.
        ...
        # Map active logical positions to physical positions using the
        # CURRENT compacted_spans (before adding new_span).  This is
        # correct because logical_to_physical() reflects the physical
        # layout as it exists right now.
        active_physical = [
