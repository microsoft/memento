...
class KVCacheManager:
    ...
    def allocate_slots(
        ...
        # Deduct compacted tokens: after KV compaction, the physical slots
        # needed are fewer than the logical token count. Without this
        # adjustment, allocate_slots would re-allocate blocks for compacted
        # tokens that have been physically removed.
    ...
    def compact_kv_cache(
        ...
        1. Copies KV values from active physical positions to slots 0, 1, 2, ...
        2. Frees trailing blocks that are now empty
        3. Preserves positional encodings (caller must track position mapping)
