...
class BlockPool:
    ...
    def copy_kv_slots(
        ...
        This method converts slot positions to block IDs and offsets, returning
        the operations for the worker to execute on GPU tensors.

        Args:
            request_id: The request ID.
            blocks: The block list for this request.
            copy_operations: List of (src_slot, dst_slot) pairs where each
                slot is a token position. src_slot's KV values should be
                copied to dst_slot.
        ...
        Note:
            The copy operations are processed in order. For compaction where
            dst_slot < src_slot for all operations, this is safe as we never
            overwrite data before reading it.
        """
        # Convert slot-level operations to block-level operations
        # This creates a list of (src_block_id, src_offset, dst_block_id, dst_offset)
        block_level_ops: list[tuple[int, int, int, int]] = []

        for src_slot, dst_slot in copy_operations:
            src_block_idx = src_slot // block_size
            src_offset = src_slot % block_size
            dst_block_idx = dst_slot // block_size
            dst_offset = dst_slot % block_size

            src_block = blocks[src_block_idx]
            dst_block = blocks[dst_block_idx]

            block_level_ops.append((
                src_block.block_id,
                src_offset,
                dst_block.block_id,
                dst_offset,
            ...
            # Also record as event if events are enabled (for KV connectors)
            if self.enable_kv_cache_events:
                self.kv_event_queue.append(
                    KVSlotCopy(
