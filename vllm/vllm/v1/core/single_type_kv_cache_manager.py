...
class SingleTypeKVCacheManager(ABC):
    ...
    def compact_kv_cache(
        ...
        This copies KV values so that:
        - Physical slot 0 contains KV for active_physical_positions[0]
        - Physical slot 1 contains KV for active_physical_positions[1]
        - etc.

        Positional encodings (e.g., RoPE) are preserved because we're moving
        the actual KV values, not recomputing them.
        ...
        Returns:
            Tuple of (blocks_freed, copy_operations) where copy_operations is
            a list of (src_block_id, src_offset, dst_block_id, dst_offset).
        """
        ...
        # and dst_slot is where it should be moved to
        ...
        # Convert to block-level operations for the worker
        ...
        # Calculate how many blocks we need after compaction
    ...
    def _get_kv_copy_ops(
        ...
        Args:
            request_id: The request ID.
            blocks: The block list for this request.
            copy_operations: List of (src_slot, dst_slot) pairs.

        Returns:
            List of (src_block_id, src_offset, dst_block_id, dst_offset) tuples.
        """
        # For safety, we use a simple approach: process in reverse order
        # when src > dst (which is always true for compaction since we're
        # moving tokens to lower indices)
        #
        # Since active_positions is sorted and dst_slot < src_slot for all
        # copies during compaction, we can process in dst order safely.
        #
        # The block pool converts slot operations to block-level operations.
        return self.block_pool.copy_kv_slots(
            request_id,
            blocks,
            copy_operations,
            self.block_size,
