...
class GPUModelRunner(
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> None:
        ...
        for new_req_data in scheduler_output.scheduled_new_reqs:
            ...
            # Extract position_offsets from extra_args if present
            position_offsets = None
            if sampling_params and sampling_params.extra_args:
                position_offsets = sampling_params.extra_args.get("position_offsets")
        ...
        if scheduler_output.kv_copy_operations:
            self._execute_kv_copy_operations(scheduler_output.kv_copy_operations)

        # Apply block table truncations BEFORE appending new blocks.
        # This removes stale block IDs that were freed during compaction,
        # making room for new blocks to be appended.
        if scheduler_output.block_table_truncations:
            for req_id, num_blocks in scheduler_output.block_table_truncations.items():
        ...
        for i, req_id in enumerate(req_data.req_ids):
            ...
            # Track compacted tokens for physical seq_lens calculation
            self.input_batch.compacted_tokens_cpu[req_index] = \
                req_data.compacted_token_counts[i]
            if new_block_ids is not None:
            ...
            if not is_last_rank:
                # Add new_token_ids to token_ids_cpu at LOGICAL positions.
                # token_ids_cpu is NOT compacted; only the KV cache is compacted.
                # This matches _gather_model_output which uses num_tokens_no_spec.
    ...
    def _execute_kv_copy_operations(
        ...
        """Execute KV cache copy operations for compaction.

        This physically copies KV values between slots to compact the cache
        after token span removal.

        Args:
            kv_copy_operations: Maps request_id to list of copy operations.
                Each operation is (src_block_id, src_offset, dst_block_id, dst_offset).
        """
        if not self.kv_caches:
            return

        # Collect all copy operations (they're the same for all requests
        # since block IDs are unique across the pool)
        ...
        # Execute copies on each layer's KV cache
        # Layout depends on attention backend:
        ...
        for kv_cache in self.kv_caches:
            if kv_cache.shape[0] == 2:
                # FlashAttention layout: dim 0 is key/value split
                key_cache = kv_cache[0]
                value_cache = kv_cache[1]
                for src_block_id, src_offset, dst_block_id, dst_offset in all_ops:
                    key_cache[dst_block_id, dst_offset].copy_(
                        key_cache[src_block_id, src_offset]
                    )
                    value_cache[dst_block_id, dst_offset].copy_(
                        value_cache[src_block_id, src_offset]
                    )
            else:
                # FlashInfer layout: dim 1 is key/value split
                for src_block_id, src_offset, dst_block_id, dst_offset in all_ops:
                    kv_cache[dst_block_id, :, dst_offset].copy_(
                        kv_cache[src_block_id, :, src_offset]
                    )
    ...
    def _prepare_inputs(
        ...
        # Apply per-token position offsets for restart mode.
        # position_offsets maps sequential position -> original position via:
        #   original_position = sequential_position + offset[sequential_position]
        offset = 0
        for i in range(num_reqs):
            req_state = self.requests[self.input_batch.req_ids[i]]
            if req_state.position_offsets is not None:
                start = int(cu_num_tokens[i - 1]) if i > 0 else 0
                end = int(cu_num_tokens[i])
                for j in range(start, end):
                    seq_pos = int(positions_np[j])
                    if seq_pos < len(req_state.position_offsets):
                        positions_np[j] += req_state.position_offsets[seq_pos]
        ...
        # Get token indices for reading from token_ids_cpu.
        # E.g., [0, 1, 0, 1, 2, 3, 4, 0, 1, 2]
        # -> [0, 1, M, M + 1, M + 2, M + 3, M + 4, 2 * M, 2 * M + 1, 2 * M + 2]
        # where M is the max_model_len.
        # NOTE: token_ids_cpu uses LOGICAL positions (not compacted).
        # Only the KV cache is compacted; token_ids_cpu keeps tokens at
        # their original logical positions. This is required for async
        # scheduling where the "margin" between positions and num_tokens
        # is needed to avoid race conditions.
        token_indices = (
            positions_np + req_indices * self.input_batch.token_ids_cpu.shape[1]
        )

        # Compute physical positions for KV cache access (slot_mapping, seq_lens).
        # After compaction, logical positions != physical positions:
        # - Logical positions (positions_np): used for RoPE and token_ids_cpu
        # - Physical positions: used for KV cache slot_mapping and seq_lens
        compacted_per_position = self.input_batch.compacted_tokens_cpu[req_indices]
        physical_positions_np = positions_np - compacted_per_position
        ...
        # Use physical positions (computed earlier) for slot_mapping.
        self.input_batch.block_table.compute_slot_mapping(
            req_indices, physical_positions_np
        )
        ...
        # seq_lens uses physical token count (logical - compacted)
        # so attention only accesses valid KV cache entries
        self.seq_lens.np[:num_reqs] = (
            self.input_batch.num_computed_tokens_cpu[:num_reqs]
            - self.input_batch.compacted_tokens_cpu[:num_reqs]
        ...
        # After compaction, seq_lens is physical (reduced), but num_tokens is logical.
        # We need to compare physical counts consistently.
        num_tokens_physical = (
            num_tokens_np - self.input_batch.compacted_tokens_cpu[:num_reqs]
        )
        self.discard_request_mask.np[:num_reqs] = (
            self.seq_lens.np[:num_reqs] < num_tokens_physical
        )
        
        # Block masking: Also skip sampling for requests with pending compactions
        # This ensures the first decode token is generated AFTER compaction
        if scheduler_output.skip_sampling_req_ids:
            for req_id in scheduler_output.skip_sampling_req_ids:
